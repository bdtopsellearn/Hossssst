...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  BUTTON COLOR SYSTEM — সব বাটন অটো কালার পাবে
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

def _btn_style(text: str) -> str:
    """বাটনের text দেখে অটো color বেছে নেয়।"""
    if not text:
        return "primary"
    t = text.lower()
    # 🔴 লাল — cancel/delete/reject
    if any(k in t for k in [
        "cancel","বাতিল","reject","রিজেক্ট","remove","delete","ডিলিট",
        "stop","exit","close","back","পিছনে","ব্যাক","❌","🗑️",
    ]):
        return "danger"
    # 🟢 সবুজ — approve/buy/deposit/positive
    if any(k in t for k in [
        "approve","অ্যাপ্রুভ","confirm","কনফার্ম","submit","buy","কিনুন",
        "deposit","ডিপোজিট","pay","পেমেন্ট",
        "add","earn","ইনকাম","wallet","balance","ব্যালেন্স",
        "shop","শপ","✅","🟢","💰","💎","🛒","purchase",
    ]):
        return "success"
    # 🔵 নীল — বাকি সব
...
class InlineKeyboardButton(_OrigInlineKeyboardButton):
    def __init__(self, text, *args, style=None, **kwargs):
        _s = style or _btn_style(str(text))
        _ak = kwargs.pop("api_kwargs", None) or {}
        _ak["style"] = _s
        super().__init__(text, *args, api_kwargs=_ak, **kwargs)
...
def _find_button_style(text: str) -> str:
    ...
    # 🟡 Warning & Withdraw:
    if any(k in t for k in [
        "পেন্ডিং", "shortage", "ঘাটতি", "অপেক্ষমান", "⏳", "⚠️"
    ]):
        return "warning"
...
async def confirm_buy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Secret text — শুধু কেনার পরেই ইউজার দেখতে পাবে
    if prod.get("secret_text"):
        await ctx.bot.send_message(
            chat_id=uid,
            text=(
                f"🔒 <b>Secret Information — শুধু আপনার জন্য:</b>\n"
                f"{divider()}\n"
                f"<code>{prod.get('secret_text')}</code>\n"
                f"{divider()}\n"
                f"⚠️ <i>এই তথ্য কারো সাথে শেয়ার করবেন না।</i>"
            ),
            parse_mode=ParseMode.HTML
        )
...
async def admin_panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "owner":
        ...
        role_badge = "🛠️ MANAGER"
        role_color = "🔵"
    else:
        role_badge = "👁️ VIEWER"
        role_color = "⚪"

    fj_on    = s.get("force_join_enabled", True)
    dep_on   = s.get("deposit_open", True)
    
    users       = fb_get("users", {}) or {}
    cats        = fb_get("categories", {}) or {}
    deposits    = fb_get("deposits", {}) or {}
    total_prod  = sum(len((c or {}).get("products", {})) for c in cats.values()) if cats else 0
    pending_dep = sum(1 for d in deposits.values() if isinstance(d,dict) and d.get("status")=="pending")

    name  = html.escape(update.effective_user.full_name or "Admin")
    ...
    text = (
        ...
        f"{role_color} পদমর্যাদা: <b>{role_badge}</b>\n\n"
        f"━━━━ 📊 লাইভ স্ট্যাটাস ━━━━\n"
        f"👥 মোট ইউজার:       <b>{len(users)}</b> জন\n"
        f"📦 প্রোডাক্ট:         <b>{total_prod}</b> টি\n"
        f"💳 পেন্ডিং ডিপোজিট:  <b>{pending_dep}</b> টি\n"
        f"💳 পেন্ডিং ডিপোজিট:  <b>{pending_dep}</b> টি\n"
        f"━━━━ 🔧 সিস্টেম অবস্থা ━━━━\n"
        f"{'🟢' if fj_on else '🔴'} Force Join:  {'চালু' if fj_on else 'বন্ধ'}\n"
        f"{'🟢' if dep_on else '🔴'} ডিপোজিট:    {'চালু' if dep_on else 'বন্ধ'}\n"
        f"🟢 Firebase:  রিয়েলটাইম সিঙ্কড\n\n"
        f"📌 নিচ থেকে বিভাগ বেছে নিন:"
    )

    dep_label  = f"💳 ডিপোজিট {'🔴 '+str(pending_dep) if pending_dep else ''}"

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒  প্রোডাক্ট ও ক্যাটাগরি", callback_data="adm_products")],
        [InlineKeyboardButton("👥 ইউজার লিস্ট",     callback_data="adm_users"),
         InlineKeyboardButton("⛔ ব্যান / আনব্যান",  callback_data="adm_ban")],
        [InlineKeyboardButton(dep_label,             callback_data="adm_deposits"),
         Inline],
        [InlineKeyboardButton("💰 ব্যালেন্স অ্যাড",  callback_data="adm_addbal"),
         InlineKeyboardButton("💳 পেমেন্ট মেথড",     callback_data="adm_payments")],
        [InlineKeyboardButton("⚙️ বট সেটিংস",        callback_data="adm_settings"),
         InlineKeyboardButton("👑 এডমিন রোল",         callback_data="adm_roles")],
        [InlineKeyboardButton("🎛️ বাটন ম্যানেজার",   callback_data="adm_btnmgr"),
         InlineKeyboardButton("📢 Force Join",         callback_data="adm_forcejoin")],
        [InlineKeyboardButton("📣 ব্রডকাস্ট",         callback_data="adm_broadcast"),
         InlineKeyboardButton("📊 স্ট্যাটিস্টিকস",   callback_data="adm_stats")],
        [InlineKeyboardButton("🔥 Firebase স্ট্যাটাস",callback_data="adm_fbstatus"),
         InlineKeyboardButton("💾 DB ব্যাকআপ",        callback_data="adm_dbbackup")],
        [InlineKeyboardButton("🔗 Check Data",         callback_data="adm_checkdata"),
         InlineKeyboardButton("⚠️ ব্যালেন্স রিসেট",  callback_data="adm_resetbal_ask")],
        [InlineKeyboardButton("🏠  মেইন মেনুতে ফিরুন", callback_data="home")],
    ])
...
async def adm_prod_detail(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton("🔒 Secret Text এডিট", callback_data=f"aedt_secret_{cat_id}__{pid}"),
...
@admin_only
async def aedt_secret_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Secret Text এডিট করার callback"""
    query = update.callback_query
    await query.answer()
    raw = query.data.replace("aedt_secret_","")
    cat_id, pid = raw.split("__",1) if "__" in raw else raw.split("_",1)
    prod = fb_get(f"categories/{cat_id}/products/{pid}", {})
    current = prod.get("secret_text","")
    ctx.user_data["awaiting"] = f"aedt_secret_{cat_id}__{pid}"
    await safe_edit_text(query,
        f"🔒 <b>SECRET TEXT এডিট করুন</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"এই text শুধু product কেনার পরেই ইউজারকে দেওয়া হবে।\n\n"
        f"<b>যা লেখা যাবে:</b>\n"
        f"• রান কমান্ড / Setup instructions\n"
        f"• License key বা activation code\n"
        f"• গোপন config বা environment variables\n"
        f"• যেকোনো গুরুত্বপূর্ণ তথ্য\n\n"
        f"📌 <b>বর্তমান secret text:</b>\n"
        f"<code>{current if current else 'এখনো সেট করা হয়নি'}</code>\n\n"
        f"✏️ নতুন secret text লিখুন (মুছতে চাইলে শুধু - পাঠান):",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ বাতিল", callback_data=f"adm_prod_{cat_id}__{pid}")]])
    )
...
async def admin_text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Settings text inputs
        ...
        ctx.user_data["awaiting"] = None
        # Type conversion
        if field in ["exchange_rate","min_deposit","referral_pct"]:
            try:
                val = float(text)
            except:
    ...
    # Stock addition for existing products
        ...
        if update.message.document:
            ...
            # Caption-এ কিছু লিখলে সেটা secret_text হিসেবে save করি
            if caption_text:
                update_data["secret_text"] = caption_text

            fb_update(f"categories/{cat_id}/products/{pid}", update_data)
            ctx.user_data["awaiting"] = None

            reply = (
                f"✅ <b>ফাইল আপলোড সম্পন্ন!</b>\n"
                f"📁 File: <code>{file_name}</code>\n"
                f"📦 Stock: 999\n"
            )
            if caption_text:
                reply += f"🔒 Secret Text: সংরক্ষণ করা হয়েছে ✅\n"
            reply += (
                f"\n💡 <i>ফাইলের সাথে secret text যোগ করতে চাইলে "
                f"ফাইল পাঠানোর সময় caption-এ লিখুন।</i>"
            )
            await update.message.reply_text(reply, parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("⚙️ প্রোডাক্ট এডিটর", callback_data=f"adm_prod_{cat_id}__{pid}")]]))

        # ── শুধু টেক্সট পাঠালে ───────────────────────────────
        elif text and text.strip():
            # Secret text আলাদা করে save করার option
            # Format: "SECRET:" দিয়ে শুরু হলে secret_text হিসেবে save হবে
            if text.upper().startswith("SECRET:"):
                secret = text[7:].strip()
                fb_update(f"categories/{cat_id}/products/{pid}", {"secret_text": secret})
                ctx.user_data["awaiting"] = None
                await update.message.reply_text(
                    f"🔒 <b>Secret Text সংরক্ষণ করা হয়েছে!</b>\n"
                    f"<i>এই text শুধু product কেনার পরেই ইউজার দেখতে পাবে।</i>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("⚙️ এডিটরে ফিরুন", callback_data=f"adm_prod_{cat_id}__{pid}")]]))
            else:
                # সাধারণ stock text items
                ...
                ctx.user_data["awaiting"] = None
                await update.message.reply_text(
                    f"✅ {len(new_items)} টি item যোগ হয়েছে। মোট stock: {len(merged)}\n\n"
                    f"💡 <i>Secret text যোগ করতে 'SECRET:' দিয়ে শুরু করে পাঠান।\n"
                    f"যেমন: SECRET:এখানে গোপন রান কমান্ড লিখুন</i>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("⚙️ এডিটরে ফিরুন", callback_data=f"adm_prod_{cat_id}__{pid}")]]))
        else:
            await update.message.reply_text(
                "⚠️ ফাইল বা টেক্সট পাঠান।\n"
                "📤 ফাইল: যেকোনো .zip/.py/.js/.txt ফাইল\n"
                "🔒 Secret text: 'SECRET:গোপন টেক্সট' লিখে পাঠান"
            )

    # Script/Product Editor: Secret Text
    elif awaiting and awaiting.startswith("aedt_secret_"):
        parts = awaiting.replace("aedt_secret_","").split("__",1)
        cat_id, pid = parts[0], parts[1]
        ctx.user_data["awaiting"] = None
        if text.strip():
            fb_update(f"categories/{cat_id}/products/{pid}", {"secret_text": text.strip()})
            await update.message.reply_text(
                f"🔒 <b>Secret Text সংরক্ষণ করা হয়েছে!</b>\n"
            ...
            # খালি পাঠালে secret text মুছে দেওয়া হবে
            fb_update(f"categories/{cat_id}/products/{pid}", {"secret_text": None})
            await update.message.reply_text(
                f"🗑️ <b>Secret Text মুছে ফেলা হয়েছে।</b>",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("⚙️ এডিটরে ফিরুন", callback_data=f"adm_prod_{cat_id}__{pid}")]]))
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    is_menu_btn = any(btn in ntxt for btn in [
        "BUY PRODUCT", "DEPOSIT MONEY", "REFER", "MY PRODUCT",
    ...
    # User withdrawal text inputs MUST take priority so admins & users can withdraw seamlessly        return
...
async def callback_router(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("aedt_secret_"):
        await aedt_secret_callback(update, ctx)
